...
import { logoutUser, checkTokenExpiration } from '../../../redux/LoginSlice';
...
const AdminSidebar = ({ setActiveTab, activeTab }) => {
  ...
  const handleLogout = async () => {
    try {
      ...
      // Still navigate to home page even if logout API fails
      navigate('/');
    }
  };
    ...
    <>
      ...
      >
        ...
        <div className="px-6 py-6">
          <h2 className="text-2xl font-mono">Lives Care</h2>
        </div>

        {/* Navigation Menu */}
        <nav className="flex-grow overflow-y-auto px-4 py-2 scrollbar-hide">
          {menuItems.map((item) => {

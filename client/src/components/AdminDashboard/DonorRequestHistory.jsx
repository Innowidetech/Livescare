...
import { fetchDonors, setSelectedView, updateDonorStatus } from '../../redux/donorSlice';
import { MapPin, Phone, Mail, Clock, ChevronDown, Filter, X, Pencil, Calendar } from 'lucide-react';
...
function StatusDropdown({ currentStatus, onStatusChange, align = 'left' }) {
...
function FilterDropdown({ options, value, onChange, align = 'left', isDate = false }) {
  ...
  if (isDate) {
    return (
      <div className="relative" ref={dropdownRef}>
        <div className="flex items-center gap-2 md:gap-2 md:px-4 md:py-2 text-[#808080] border border-[#E5E5E5] rounded-lg text-xs md:text-lg">
          <DatePicker
            selected={value}
            onChange={onChange}
  ...
  return (
    <div className="relative" ref={dropdownRef}>
      <button
        onClick={() => setIsOpen(!isOpen)}
        className="flex items-center gap-2 md:gap-2 md:px-4 md:py-2 text-[#808080] border border-[#E5E5E5] rounded-lg text-xs md:text-lg"
      >
        <span>{value}</span>
...
function DonorRequestHistory() {
  const dispatch = useDispatch();
  const { donors, status, error, selectedView } = useSelector((state) => state.donor);
  ...
  const handleStatusChange = async (donorId, newStatus) => {
    try {
      await dispatch(updateDonorStatus({ donorId, newStatus })).unwrap();
    } catch (error) {
        ...
        <div className="">
          <div className="flex flex-col gap-6">
            <div className="flex justify-between items-center">
              <h1 className="md:text-2xl font-semibold text-[#202224]">Donor Request</h1>
            ...
            <div className="hidden lg:block rounded-xl shadow">
              <table className="w-full">
                <thead>
                  <tr className="border-b bg-[#F1F4F9]">
                    <th className="px-6 py-4 text-left font-medium text-[#202224]">Name</th>
                    <th className="px-6 py-4 text-left font-medium text-[#202224]">Item Name</th>
                    <th className="px-6 py-4 text-left font-medium text-[#202224]">No.Of Items</th>
                    <th className="px-6 py-4 text-left font-medium text-[#202224]">Date</th>
                    ...
                    <th className="px-6 py-4 text-left font-medium text-[#202224]"></th>
                    <th className="px-6 py-4 text-left font-medium text-[#202224]">Status</th>
                  </tr>
                </thead>
                <tbody>
                  {filteredDonors.filter((item) => item.status != "Pending" ).map((donation) => (
                    <tr key={donation.id} className="border-b last:border-b-0">
                      <td className="px-6 py-4 text-[#202224]">{donation.name}</td>
                      <td className="px-6 py-4 text-[#202224]">{donation.itemName}</td>
                      <td className="px-6 py-4 text-[#202224]">{donation.count}</td>
                      <td className="px-6 py-4 text-[#202224]">
                        {new Date(donation.createdAt).toLocaleDateString()}
                      </td>
                      <td className="px-6 py-4 text-[#202224]">{donation[selectedView.place] || 'N/A'}</td>
                      <td className="px-6 py-4 text-[#202224]">{donation[selectedView.contact] || 'N/A'}</td>
                ...
                <div key={donation.id} className="bg-[#FCFDFD] rounded-xl p-4 space-y-4">
                  <div className="space-y-2">
                    <div className="grid">
                      ...
                      <div className='flex items-center justify-between'>
                        <h1 className="text-left font-medium text-[#202224] text-sm">No Of Items</h1>
                        <h1 className="py-4 text-[#202224]">{donation.count}</h1>
                      </div>
                      <div className='flex items-center justify-between'>
                      ...
                      <div className='flex items-center justify-between'>
                        <h1 className="text-left font-medium text-[#202224]"></h1>
                        <button
                          onClick={() => handleOpenModal(donation.description)}

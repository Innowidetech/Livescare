...
try {
        ...
        const { issuedTo, deliveredDate, status} = req.body;
        if(!issuedTo || !deliveredDate || !status){ return res.status(400).json({message:"Provide the following details to issue certificate."})}
        if(!certificateId){
